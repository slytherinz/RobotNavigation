...
(function(g, U, J) {
	...
	function Wa(d) {
		var c = 3,
			a;
		"array" !== aa(d.start) && (a = d.start.slice(0),
			d.start = Ha(d.start), console.log(a, "=>", d.start), d.end = Ha(d.end));
		d.now = [];
	...
	function T(d, c, a) {
		var b, f, e, h;
		if (a = a._args) d = H(d), b = d.event, null !== b.x && null !== b.y && (e = b.x * d.pixelRatio, h = b.y * d.pixelRatio, f = c.isPointInPath(e, h) || c.isPointInStroke && c.isPointInStroke(e, h)), c = d.transforms, a.eventX = b.x, a.eventY = b.y, a.event =
			b.event, b = d.transforms.rotate, e = a.eventX, h = a.eventY, 0 !== b ? (a._eventX = e * L(-b) - h * P(-b), a._eventY = h * L(-b) + e * P(-b)) : (a._eventX = e, a._eventY = h), a._eventX /= c.scaleX, a._eventY /= c.scaleY, f && d.intersecting.push(a), a.intersects = !!f
	}
	...
	function La(d, c, a, b, f, e, h, g) {
		...
		b.arrowRadius && !a.closed && (t = $a(g - e, h - f), t -= F, d = a.strokeWidth * L(t), n = a.strokeWidth * P(t), a = h + b.arrowRadius * L(t + b.arrowAngle / 2), f = g + b.arrowRadius * P(t + b.arrowAngle / 2), e = h + b.arrowRadius * L(t - b.arrowAngle / 2), b = g + b.arrowRadius * P(t - b.arrowAngle / 2),
			c.moveTo(a - d, f - n), c.lineTo(h - d, g - n), c.lineTo(e - d, b - n), c.moveTo(h - d, g - n), c.lineTo(h + d, g + n), c.moveTo(h, g))
	}
	...
	function Ma(d, c, a, b) {
		...
		for (void 0 !== b.x1 && void 0 !== b.y1 && c.moveTo(b.x1 +
				a.x, b.y1 + a.y);;)
	...
	function Na(d, c, a, b) {
		...
		for (void 0 !== b.x1 && void 0 !== b.y1 && c.moveTo(b.x1 + a.x, b.y1 + a.y);;)
			...
			--f;
		ha(d,
			c, a, b, b["cx" + (f - 1)] + a.x, b["cy" + (f - 1)] + a.y, b["x" + f] + a.x, b["y" + f] + a.y)
	}
	...
	function Oa(d, c, a, b) {
		...
		for (void 0 !== b.x1 && void 0 !== b.y1 && c.moveTo(b.x1 + a.x, b.y1 + a.y);;)
			...
			--f;
		e -= 2;
		ha(d, c, a, b, b["cx" + (e + 1)] + a.x, b["cy" +
			(e + 1)] + a.y, b["x" + f] + a.x, b["y" + f] + a.y)
	}
	...
	function ta(d, c, a, b) {
		...
		if (f.text === a.text && f.fontStyle === a.fontStyle && f.fontSize === a.fontSize && f.fontFamily === a.fontFamily && f.maxWidth === a.maxWidth && f.lineHeight === a.lineHeight) a.width = f.width, a.height = f.height;
			...
			a.height = ba(g.css(d, "fontSize")) * b.length *
				a.lineHeight;
			d.style.fontSize = c
		}
	}
				...
				if (n.drag.layer) {
					...
					if ("mousemove" === C || "touchmove" === C) {
						if (k.dragging || (k.dragging = !0, m.dragging = !0, m.bringToFront && (u.splice(m.index, 1), m.index = u.push(m)), m._startX = m.x, m._startY =
								m.y, m._endX = m._eventX, m._endY = m._eventY, O(e, x, m, "dragstart")), k.dragging)
						...
						C || "touchend" === C) k.dragging && (m.dragging = !1, k.dragging = !1, O(e, x, m, "dragstop")), x.drag = {}
				}
				e = n.lastIntersected;
		...
		for (h = 0; h < this.length; h += 1)
			if (g = K(this[h]))
				for (t = H(this[h]), n = new J(e), N(this[h], n, e, f), z = 0; z < n.count; z += 1) {
					var E = g,
						x = t;
					0 === x.savedTransforms.length ? x.transforms =
						la(oa) : (E.restore(), x.transforms = x.savedTransforms.pop())
				}
			return this
	};
	...
	g.fn.drawRect = function n(g) {
		...
		for (z = 0; z < this.length; z += 1)
			if (E = K(this[z])) x = new J(g), N(this[z], x, g, n), x.visible && (Q(this[z], E, x, x.width, x.height), S(this[z], E, x), E.beginPath(), x.width && x.height && (C = x.x - x.width / 2, u = x.y - x.height / 2, (y = bb(x.cornerRadius)) ? (m = x.x + x.width / 2, k = x.y + x.height / 2, 0 > x.width && (B = C, C = m, m = B), 0 > x.height && (B = u, u = k, k = B), 0 > m - C - 2 * y && (y = (m - C) / 2), 0 > k - u - 2 * y &&
				(y = (k - u) / 2), E.moveTo(C + y, u), E.lineTo(m - y, u), E.arc(m - y, u + y, y, 3 * F / 2, 2 * F, !1), E.lineTo(m, k - y), E.arc(m - y, k - y, y, 0, F / 2, !1), E.lineTo(C + y, k), E.arc(C + y, k - y, y, F / 2, F, !1), E.lineTo(C, u + y), E.arc(C + y, u + y, y, F, 3 * F / 2, !1), x.closed = !0) : E.rect(C, u, x.width, x.height)), T(this[z], E, x), W(this[z], E, x));
		return this
	};
	g.fn.drawArc = function t(g) {
		var E, x, C;
		for (E = 0; E < this.length; E += 1)
			if (x = K(this[E])) C = new J(g), N(this[E], C, g, t), C.visible && (Q(this[E], x, C, 2 * C.radius), S(this[E], x, C), x.beginPath(), Ka(this[E], x, C, C), T(this[E], x, C), W(this[E],
				x, C));
		return this
	};
	...
	g.fn.drawSlice = function x(C) {
		...
		for (u = 0; u < this.length; u += 1)
			if (g(this[u]), m = K(this[u])) k = new J(C), N(this[u],
				k, C, x), k.visible && (Q(this[u], m, k, 2 * k.radius), S(this[u], m, k), k.start *= k._toRad, k.end *= k._toRad, k.start -= F / 2, k.end -= F / 2, k.start = Ja(k.start), k.end = Ja(k.end), k.end < k.start && (k.end += 2 * F), y = (k.start + k.end) / 2, B = k.radius * k.spread * L(y), y = k.radius * k.spread * P(y), k.x += B, k.y += y, m.beginPath(), m.arc(k.x, k.y, k.radius, k.start, k.end, k.ccw), m.lineTo(k.x, k.y), T(this[u], m, k), k.closed = !0, W(this[u], m, k));
		return this
	};
	g.fn.drawLine1 = function C(g) {
		var m, k, y;
		for (m = 0; m < this.length; m += 1)
			if (k = K(this[m])) y = new J(g), N(this[m], y,
				g, C), y.visible && (Q(this[m], k, y), S(this[m], k, y), k.beginPath(), Ma(this[m], k, y, y), T(this[m], k, y), W(this[m], k, y));
		return this
	};
	...
	g.fn.drawBezier = function m(g) {
		...
		for (y = 0; y < this.length; y += 1)
			if (B = K(this[y])) w = new J(g), N(this[y], w, g, m), w.visible && (Q(this[y], B, w), S(this[y],
				B, w), B.beginPath(), Oa(this[y], B, w, w), T(this[y], B, w), W(this[y], B, w));
		return this
	};
		...
		for (w = 0; w < this.length; w += 1)
			if (A = K(this[w]))
				if (p = new J(g), N(this[w], p, g, y), p.visible) {
					...
					for (l = 1;;)
						if (v = p["p" +
								l], void 0 !== v) v = new J(v), "line" === v.type ? Ma(this[w], A, p, v) : "quadratic" === v.type ? Na(this[w], A, p, v) : "bezier" === v.type ? Oa(this[w], A, p, v) : "vector" === v.type ? Ra(this[w], A, p, v) : "arc" === v.type && Ka(this[w], A, p, v), l += 1;
						else break;
					T(this[w], A, p);
					W(this[w], A, p)
				}
				...
				if (l = new J(w), v = N(this[A], l, w, B), l.visible) {
					...
					if (l.radius)
						for (G = ba(l.fontSize), null === l.letterSpacing && (l.letterSpacing = G / 500), v = 0; v < D.length; v += 1) {
							...
							for (v = 0; v < D.length; v += 1) r = D[v], R = l.y + v * l.height / D.length - (D.length - 1) * l.height / D.length / 2, p.shadowColor = l.shadowColor, p.fillText(r, G, R), "transparent" !== l.fillStyle && (p.shadowColor = "transparent"),
	...
	g.fn.drawImage = function w(A) {
		function p(l, w, p, q, r) {
			return function() {
				...
				null !== q.sWidth && null !== q.sHeight && null !== q.sx && null !== q.sy ? (null === q.width && (q.width = q.sWidth), null === q.height && (q.height = q.sHeight), q.cropFromCenter && (q.sx += q.sWidth / 2, q.sy += q.sHeight / 2), 0 > q.sy - q.sHeight / 2 && (q.sy = q.sHeight / 2), q.sy + q.sHeight / 2 > I.height && (q.sy = I.height - q.sHeight / 2), 0 > q.sx - q.sWidth / 2 && (q.sx = q.sWidth / 2), q.sx + q.sWidth /
					2 > I.width && (q.sx = I.width - q.sWidth / 2), Q(l, w, q, q.width, q.height), S(l, w, q), w.drawImage(I, q.sx - q.sWidth / 2, q.sy - q.sHeight / 2, q.sWidth, q.sHeight, q.x - q.width / 2, q.y - q.height / 2, q.width, q.height)) : (Q(l, w, q, q.width, q.height), S(l, w, q), w.drawImage(I, q.x - q.width / 2, q.y - q.height / 2, q.width, q.height));
				w.beginPath();
	...
	g.fn.createGradient = function(g) {
		...
		if (A = K(this[0])) {
			...
			g.x2 = g.x2 || 0;
			g.y2 = g.y2 || 0;
			A = null !== g.r1 && null !== g.r2 ?
				A.createRadialGradient(g.x1, g.y1, g.r1, g.x2, g.y2, g.r2) : A.createLinearGradient(g.x1, g.y1, g.x2, g.y2);
	...
	g.fn.detectPixelRatio = function(A) {
		var p, l, v, D, r, G, F;
		for (l = 0; l < this.length; l += 1) p = this[l], g(this[l]), v = K(p), F = H(this[l]), F.scaled || (D = U.devicePixelRatio || 1, r = v.webkitBackingStorePixelRatio || v.mozBackingStorePixelRatio || v.msBackingStorePixelRatio || v.oBackingStorePixelRatio || v.backingStorePixelRatio || 1, D /= r, 1 !== D && (r = p.width, G = p.height, p.width = r * D, p.height =
